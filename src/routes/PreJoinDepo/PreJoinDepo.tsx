...
const PreJoinDepo = () => {
    ...
    const { loginUser, loading: loginLoading, loginError, addParticipantError, isLogged } = useLogin(depositionID);
    ...
    const showWaitingRoom = async () => {
        await checkUserStatus(depositionID, emailRef.current);
        setWaiting(true);
    };
    ...
    const verifyUserStatus = async (email: string) => {
        ...
        if (tempToken) localStorage.removeItem(TEMP_TOKEN);
    ...
    return (
        ...
        >
            {waiting ? (
                <WaitingRoom userStatus={userStatus} />
            ) : (
                <Wizard
